...
private:
  ...
  class PairComp{
    private:
      Key_compare comp;
    public:
      bool operator()(const Pair_type data1, const Pair_type data2) const{
        return comp(data1.first, data2.first);
      }
  };
...
public:
  ...
  Iterator find(const Key_type& k) const{
    Pair_type temp;
    temp.first = k;
    temp.second = Value_type();
    Iterator it = tree.find(temp);
    return it;
  }
  ...
  Value_type& operator[](const Key_type& k){
    if(find(k) == tree.end()){
      std::pair <Key_type, Value_type> temp;
      temp.first = k;
      temp.second = Value_type();
      insert(temp);
    }
  ...
  std::pair<Iterator, bool> insert(const Pair_type &val){
    ...
    if(tree.find(val) == end()){
       temp.first = tree.insert(val);
       temp.second = true;
       return temp;
    }
    else{
      temp.first = tree.find(val);
      temp.second = false;
      return temp; 
    }
  }
