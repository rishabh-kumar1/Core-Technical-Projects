...
// ---------- DO NOT CHANGE ANYTHING IN THIS FILE ABOVE THIS LINE ----------
  ...
  static int height_impl(const Node *node) {
    if(node == nullptr){
      ...
      int leftH = height_impl(node->left);
      int rightH = height_impl(node->left);
      if(leftH >= rightH){
        return leftH + 1;
      }
      else{
  ...
  static Node *copy_nodes_impl(Node *node) {
    if(node == nullptr){
      ...
      otherNode->datum = node->datum;
      otherNode->right = copy_nodes_impl(node->right);
      otherNode->left = copy_nodes_impl(node->left);
      return otherNode;
    }
  ...
  static Node * find_impl(Node *node, const T &query, Compare less) {
    ...
    if(!less(query, node->datum) && !less(node->datum, query)){
      ...
      if(node->right != nullptr && less(node->datum, query)){
        return find_impl(node->right, query, less);
      }
      else if(node->right != nullptr && less(query, node->datum)){
